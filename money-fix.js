// SNYDER GOLF v4.39 League Money display repair
...
(function(){
  ...
  function isPlayerRow(row){
    ...
    // The original League row has a hidden entry cell, but v4.39 removes it visually.
  ...
  function ensureEntryNote(table){
    if(!table||table.getAttribute('data-money-entry-note')==='v4.39')return;
    table.setAttribute('data-money-entry-note','v4.39');
    try{
  ...
  function patch(){
    ...
    try{
      ...
      headers.forEach(function(header){
        ...
        kids(table).forEach(function(row){
          ...
          // v4.39: make the repair idempotent. v4.16 overwrote the Entry cell with Rounds,
          ...
          if(row.getAttribute('data-money-base-rounds')!==null){
            ...
            var alreadyRepaired=row.getAttribute('data-money-repaired')==='v4.39' || rowText.indexOf(' sweep ')!==-1 || cleanText(original[3]).toLowerCase().indexOf('sweep')!==-1;
            if(alreadyRepaired){
          ...
          row.setAttribute('data-money-repaired','v4.39');
    ...
    }catch(e){console.warn('Snyder money fix v4.39 skipped safely',e);}
  }
  function schedule(){
    if(scheduled)return;
    scheduled=true;
