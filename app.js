...
async function sendSnyderLiveNotification(type,payload){
  try{
    ...
    const body={type,app:'snyder-live',subscriptionTable:SNYDER_PUSH_TABLE,version:'v4.66',createdAt:new Date().toISOString(),...(payload||{})};
...
async function sendSnyderLeagueNotification(payload){
  try{
    const body={type:'league_score_submitted',app:'snyder-live',source:'snyder-league',subscriptionTable:SNYDER_PUSH_TABLE,version:'v4.66',createdAt:new Date().toISOString(),...(payload||{})};
    if(body.body&&!body.message)body.message=body.body;
    if(snyderNotificationsTestMode()){
...
const WHITLEY_BAY_PRESETS=[
  {name:'Whitley Bay Golf Club - White Tee',location:'Whitley Bay',image_url:WHITLEY_BAY_BADGE,course_rating:72.3,slope_rating:131,holes:[
...
function presetShouldOverrideSavedCourseHoles(preset){
...
async function fetchDayCompRoundsFromCloud(dayKey,fallbackRounds=[]){
  ...
  for(const field of fields){
    try{
      const res=await sb.from('cup_rounds').select('*').ilike(field,'%'+marker+'%').range(0,9999);
...
function App(){
  ...
  async function loadAll(){
    const[players,courses,rounds,groups,competitions,scores,cupUsers,guests,cupEvents,cupTeams,cupEventPlayers,cupDays,cupMatches]=await Promise.all([
      ...
      sb.from('cup_rounds').select('*').order('created_at',{ascending:false}).range(0,9999).then(r=>r.data||[]),
      sb.from('cup_groups').select('*').range(0,9999).then(r=>r.data||[]),
      sb.from('cup_competitions').select('*').then(r=>r.data||[]),
      sb.from('cup_scores').select('*').range(0,9999).then(r=>r.data||[]),
    ...
    <div style={{minHeight:'100vh',paddingBottom:60,background:'linear-gradient(180deg,#0d2548 0%,#0a1f3d 100%)'}}>
      ...
      <div style={{position:'fixed',bottom:0,left:0,right:0,background:'#0d2548',borderTop:'1px solid rgba(255,255,255,0.1)',display:'flex',padding:'8px 0 20px'}}>
        ...
        <button onClick={()=>setView('admin')} style={bottomTabStyle('rgba(255,255,255,0.4)')}>
          ...
          <span onClick={tapVersionForTestMode} aria-label="App version v4.66" title="Version" style={{fontSize:8,fontWeight:700,letterSpacing:'0.06em',lineHeight:'9px',color:testMode?'#fbbf24':'rgba(255,255,255,0.32)',padding:'2px 4px',marginTop:-2}}>v4.66</span>
        </button>
      </div>
...
function LiveScoringView({rounds,groups,scores,players,courses,cupUsers,cupEvents,cupTeams,cupEventPlayers,cupDays,cupMatches,sb,flash,setView,selectedComp,activeComp,setSelectedRound,currentUser,setHoleScores,holeScores}){
  ...
  function DayLeaderboardModal({rd}){
    ...
    function scorecardForDayPlayer(playerId){
      const target=normaliseId(playerId);
      if(!target)return null;
      const roundsToCheck=(effectivePlayable&&effectivePlayable.length?effectivePlayable:playable)||[];
      const freshGroupsByRound={};
      (freshDayGroups||[]).forEach(g=>{const rid=g&&g.round_id;if(!rid)return;(freshGroupsByRound[rid]=freshGroupsByRound[rid]||[]).push(g);});
      for(const round of roundsToCheck){
        if(!round||!round.id)continue;
        const isCup=isSnyderCupRound(round);
        const roundPlayers=(freshDayRoundPlayers&&freshDayRoundPlayers[round.id])||publicRoundPlayers[round.id]||[];
        for(const raw of roundPlayers){
          const rp=mapRoundPlayerForScorecard(raw,isCup);
          const ids=[rp.id,rp.user_id,rp.guest_id,rp.cup_player_id,raw&&raw.id,raw&&raw.user_id,raw&&raw.guest_id].map(normaliseId).filter(Boolean);
          if(ids.includes(target))return round;
        }
        const roundGroups=(freshGroupsByRound[round.id]&&freshGroupsByRound[round.id].length?freshGroupsByRound[round.id]:groupsForRound(round))||[];
        for(const g of roundGroups){
          const ids=[...(g.player_ids||[]),...Object.keys(g.playing_handicaps||{})].map(normaliseId).filter(Boolean);
          if(ids.includes(target))return round;
        }
      }
      return null;
    ...
    return(
      <div style={{position:'fixed',inset:0,background:'linear-gradient(180deg,rgba(4,12,28,0.94),rgba(2,8,23,0.88))',zIndex:1200,overflow:'hidden',overscrollBehavior:'none',touchAction:'none'}}>
        ...
        <div style={{maxWidth:540,margin:'0 auto'}}>
          ...
          <div style={{...S.card,padding:14,marginBottom:8,borderColor:'rgba(96,184,240,0.42)',background:'linear-gradient(180deg,rgba(14,60,105,0.46),rgba(13,37,72,0.90))',boxShadow:'0 14px 30px rgba(0,112,187,0.16)'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'22px minmax(0,1fr) 34px 34px 40px 32px 54px',gap:5,padding:'6px 0',borderBottom:'1px solid rgba(96,184,240,0.22)',fontSize:9,color:'#90ccf0',fontWeight:950,letterSpacing:'0.05em'}}>
              <div>#</div><div>Player</div><div style={{textAlign:'right'}}>F9</div><div style={{textAlign:'right'}}>B9</div><div style={{textAlign:'right'}}>Total</div><div style={{textAlign:'right'}}>H</div><div style={{textAlign:'right'}}>Card</div>
            </div>
            {boardRows.length?boardRows.map((r,idx)=>{
              const linkedCard=scorecardForDayPlayer(r.id);
              return (
              <div key={r.id} style={{display:'grid',gridTemplateColumns:'22px minmax(0,1fr) 34px 34px 40px 32px 54px',gap:5,alignItems:'center',padding:'8px 0',borderBottom:idx===boardRows.length-1?'none':'1px solid rgba(255,255,255,0.07)',background:idx===0?'linear-gradient(90deg,rgba(245,191,36,0.13),rgba(96,184,240,0.04))':'transparent',borderRadius:idx===0?8:0}}>
                <div style={{fontSize:12,color:idx===0?'#fbbf24':'rgba(255,255,255,0.58)',fontWeight:950}}>{idx+1}</div>
                <div style={{fontSize:13,color:'#fff',fontWeight:900,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{nameFromContextMap(dayNameMap,r.id,r.name||'Player')}</div>
                ...
                <button disabled={!linkedCard} onClick={()=>linkedCard&&openRound(linkedCard)} style={{border:'1px solid '+(linkedCard?'rgba(96,184,240,0.42)':'rgba(255,255,255,0.10)'),background:linkedCard?'rgba(0,112,187,0.70)':'rgba(255,255,255,0.05)',color:linkedCard?'#fff':'rgba(255,255,255,0.38)',borderRadius:8,padding:'5px 6px',fontSize:10,fontWeight:950,textAlign:'center'}}>View</button>
              </div>
  ...
  async function connectEnglandGolf(){
    ...
    try{
      ...
      const updated={
        ...
        england_golf_sync_error:data&&data.needs_sync_confirmation?'Saved new login details. Waiting for next sync to confirm at 2:30am.':null
      };
      setCurrentUser(updated);
      try{localStorage.setItem('snyder_user',JSON.stringify(updated));}catch(e){}
      setHcp(nextHandicap);
      ...
      <div style={{padding:16}}>
        ...
        <div style={{...S.card,marginBottom:16}}>
          ...
          <div style={{fontSize:11,color:englandGolfPending?'#86efac':englandGolfProblem?'#fca5a5':'rgba(255,255,255,0.55)',marginTop:10,lineHeight:1.45}}>
            {englandGolfPending?'Saved new login details. Waiting for next sync to confirm at 2:30am.':currentUser&&currentUser.england_golf_last_sync_at?'Last sync: '+new Date(currentUser.england_golf_last_sync_at).toLocaleString('en-GB'):'Not connected yet'}
...
function groupCountForRange(range){
  ...
  if(range==='13-16')return 4;
  if(range==='more')return 5;
  return 1;
}
...
function PlayGolf({players,courses,rounds,groups,scores,sb,flash,setView,setSelectedRound,load,isAdmin,currentUser,cupUsers,guests,selectedRound,holeScores,setHoleScores,promptStartRoundAuth}){
  ...
  function resetGroupsForRange(range){
    ...
    const count=range==='more'?Math.max(5,Math.ceil(flat.length/4)||5):groupCountForRange(range);
  ...
  async function startRound(skipHandicapConfirm=false){
    ...
    const promptDayKey=promptDayBoard?dayCompKeyFromRound(promptDayBoard):'';
    const effectiveJoinKey=setup.dayCompMode==='join'?(setup.dayCompKey||promptDayKey):(((setup.dayCompMode||'none')==='none'&&promptDayKey&&dayJoinChoice!=='skip')?promptDayKey:'');
    const effectiveDayCompMode=setup.dayCompMode==='create'?'create':(effectiveJoinKey?'join':'none');
    if(effectiveDayCompMode==='create'&&(parseInt(setup.sweepstake&&setup.sweepstake.amountPence)||0)<=0){flash('Enter a sweepstake amount','error');return;}
    ...
    try{
      ...
      const dayCompKey=effectiveDayCompMode==='create'?makeDayCompKey():(effectiveDayCompMode==='join'?effectiveJoinKey:'');
      if(effectiveDayCompMode==='join'&&!dayCompKey){
        flash('Could not find the day sweepstake. Tap refresh and try Join again.','error');
        setSaving(false);
        return;
      }
      let roundName=effectiveDayCompMode==='create'?(setup.name||defaultDaySweepstakeName()):(setup.name||creatorName||(courseBaseName+' - '+today)||'Round');
      const joinedDayBoard=effectiveDayCompMode==='join'?(dayBoardByKey(dayCompKey)||promptDayBoard):null;
      ...
      if(roundSweepstake&&roundSweepstake.enabled){
        ...
        if(dayCompKey){
          ...
          if(joinedDayBoard&&joinedDayBoard.id){
      ...
      return(
        <div style={{minHeight:'100vh',paddingBottom:40}}>
          ...
          <div style={{padding:16}}>
            ...
            <button onClick={()=>{setDayJoinChoice('join');setSetup(q=>({...q,dayCompMode:'join',dayCompKey:dayCompKeyFromRound(promptDayBoard)||''}));setDayJoinPromptDone(true);}} style={{...S.pri,width:'100%',padding:14,fontSize:15,marginBottom:10}}>Yes, join it</button>
            <button onClick={()=>{setDayJoinChoice('skip');setSetup(q=>({...q,dayCompMode:'none',dayCompKey:''}));setDayJoinPromptDone(true);}} style={{...S.gho,width:'100%',padding:12,fontSize:13}}>No, normal round</button>
          </div>
        </div>
      );
    }
  ...
  if(step==='setup'){
    ...
    const mustChooseDaySweepstake=q==='day'&&!!promptDayBoard;
        ...
        <div style={{padding:16}}>
          ...
          {q==='day'&&<div style={{...S.card,marginBottom:12}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
              <button onClick={()=>{setDayJoinChoice('join');setSetup(q=>({...q,dayCompMode:'join',dayCompKey:dayCompKeyFromRound(promptDayBoard)||''}));setDayJoinPromptDone(true);setSetupQuestion('format');}} style={{...S.pri,padding:12,fontSize:13}}>Yes, join</button>
              <button onClick={()=>{setDayJoinChoice('skip');setDayJoinPromptDone(true);setSetup(q=>({...q,dayCompMode:'none',dayCompKey:''}));setSetupQuestion('format');}} style={{...S.gho,padding:12,fontSize:13}}>No thanks</button>
            </div>
          </div>}
          ...
          {q==='format'&&<div style={{display:'grid',gap:10,marginBottom:12}}>
            ...
            <div style={{...S.card,padding:12,background:selectedFormat==='groups'?'rgba(96,184,240,0.16)':'rgba(255,255,255,0.045)',borderColor:selectedFormat==='groups'?'rgba(96,184,240,0.48)':'rgba(255,255,255,0.10)'}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:7}}>
                {[['5-8','5-8'],['9-12','9-12'],['13-16','13-16'],['more','17+']].map(([r,label])=><button key={r} onClick={()=>chooseWizardPreset('normal',r)} style={{border:'1px solid '+(playerRange===r?'rgba(96,184,240,0.70)':'rgba(255,255,255,0.12)'),background:playerRange===r?'rgba(96,184,240,0.24)':'rgba(255,255,255,0.06)',color:'#fff',borderRadius:10,padding:'9px 5px',fontSize:12,fontWeight:950}}>{label}</button>)}
              </div>
            </div>
          </div>}
          ...
          {q==='players'&&<div style={{marginBottom:12}}>
            ...
            {isSingleGroupDay?(
              ...
              <>
                {playerRange==='more'&&<button onClick={()=>setGroupSetup(g=>[...(g||[]),[]])} style={{...S.gho,width:'100%',fontSize:13,padding:'10px 12px',marginBottom:10}}>+ Add another group</button>}
          ...
          {!mustChooseDaySweepstake&&<button onClick={goNext} disabled={saving||!canAdvanceSetupQuestion(q)||(isLastQuestion&&!setup.course_id)} style={{...S.pri,width:'100%',padding:14,fontSize:15,marginTop:12,opacity:saving||!canAdvanceSetupQuestion(q)||(isLastQuestion&&!setup.course_id)?0.5:1}}>
            {saving?'Starting...':nextLabel}
          </button>}
          {!mustChooseDaySweepstake&&!isLastQuestion&&<button onClick={()=>goSetupQuestion(1)} disabled={!canAdvanceSetupQuestion(q)} style={{...S.gho,width:'100%',padding:10,fontSize:12,marginTop:8,opacity:canAdvanceSetupQuestion(q)?1:0.55}}>Skip / use current answer</button>}
        </div>
        ...
        <div style={{padding:16}}>
          ...
          {!isMatchplayOnlySetup()&&<div style={{padding:'12px 14px',background:setup.dayCompMode&&setup.dayCompMode!=='none'?'rgba(96,184,240,0.13)':'rgba(255,255,255,0.06)',borderRadius:10,marginBottom:16,border:'1px solid '+(setup.dayCompMode&&setup.dayCompMode!=='none'?'rgba(96,184,240,0.38)':'rgba(255,255,255,0.12)')}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
              ...
              <select value={setup.dayCompMode==='join'?setup.dayCompKey:setup.dayCompMode||'none'} onChange={e=>{const v=e.target.value;setDayJoinChoice(v==='none'?'skip':(v==='create'?'unasked':'join'));setSetup(q=>v==='create'?{...q,dayCompMode:'create',dayCompKey:''}:v==='none'?{...q,dayCompMode:'none',dayCompKey:''}:{...q,dayCompMode:'join',dayCompKey:v});}} style={{...S.inp,width:150,marginBottom:0,padding:'8px 9px',fontSize:12}}>
          ...
          {isSingleGroupDay ? <div style={{marginBottom:12}}>
            ...
            {playerRange==='more'&&<button onClick={()=>setGroupSetup(g=>[...(g||[]),[]])} style={{...S.gho,width:'100%',fontSize:13,padding:'10px 12px',marginBottom:0}}>+ Add another group</button>}
...
function LiveScorecard({round,group,players,courses,rounds,scores,sb,flash,load,setView,holeScores,setHoleScores,currentUser}){
  ...
  const[sweepstakeLeagueBalancePreview,setSweepstakeLeagueBalancePreview]=useState({status:'idle',bySweepId:{}});
  ...
  function normalSweepstakeSettlementNotes(key){
    return ['v4.66','v4.65','v4.64','v4.63','v4.62','v4.61','v4.60','v4.59','v4.58','v4.57','v4.56','v4.55','v4.54','v4.53','v4.52','v4.51','v4.50','v4.49','v4.48','v4.47','v4.46','v4.45','v4.44','v4.43','v4.42','v4.41','v4.40','v4.39','v4.38','v4.37','v4.36','v4.35','v4.34','v4.33'].map(v=>`Sweepstake League balance settlement ${key} | adjustment-only | ${v}`);
  }
  ...
  function isLeagueSweepstakeMoneyNote(note){
    ...
    if(text.indexOf('test sweepstake ignored')!==-1||text.indexOf('exclude from league money')!==-1)return false;
    return text.indexOf('Day sweepstake League balance settlement')!==-1||text.indexOf('Day sweepstake League balance reversal')!==-1||text.indexOf('Sweepstake League balance settlement')!==-1||text.indexOf('Sweepstake League balance reversal')!==-1;
  }
  async function settleSweepstakeLeagueBalances(){
    ...
    const markerNote=normalSweepstakeSettlementNotes(key)[0];
    try{
      const {data:logMarkers,error:logMarkerError}=await sb.from('payment_log').select('id').or(normalSweepstakeSettlementNotes(key).map(n=>`note.eq.${n}`).join(',')).limit(1);
  ...
  useEffect(()=>{
    let cancelled=false;
    async function loadSweepstakeLeagueBalancePreview(){
      if(!sb||!round||!round.id||round._cupScoring||isFoursomesScorecard||dayCompKeyFromRound(round)||!isCompletedRound(round)){
        setSweepstakeLeagueBalancePreview({status:'idle',bySweepId:{}});
        return;
      }
      const sw=sweepstakePlayerRows({throughHole:18,forceEnabled:!!round._spectator});
      if(!sw.enabled||!sw.final||!sw.rows.length){
        setSweepstakeLeagueBalancePreview({status:'idle',bySweepId:{}});
        return;
      }
      setSweepstakeLeagueBalancePreview(prev=>({...prev,status:'loading'}));
      try{
        const key=sweepstakeSettlementKey();
        const thisRoundNotes=new Set(normalSweepstakeSettlementNotes(key));
        const [{data:leaguePlayers,error:playersError},{data:leagueScores,error:scoresError},{data:paymentRows,error:paymentsError},{data:snakeRows,error:snakeError},{data:logRows,error:logError},linkResult]=await Promise.all([
          sb.from('players').select('*').order('name',{ascending:true}),
          sb.from('scores').select('id,player_id,date'),
          sb.from('payments').select('player_id,paid'),
          sb.from('snake_log').select('player_id,confirmed'),
          sb.from('payment_log').select('player_id,player_name,action,amount,note').or('action.eq.Sweepstake balance,action.eq.Sweepstake balance reversal').limit(1000),
          fetchLeaguePlayerLinks(sb)
        ]);
        const err=playersError||scoresError||paymentsError||snakeError||logError;
        if(err)throw err;
        const links=(linkResult&&linkResult.links)||{};
        const scoreCount={}, snakeCount={}, paidById={}, sweepNetPence={};
        (leagueScores||[]).forEach(s=>{const id=normaliseId(s&&s.player_id);if(id)scoreCount[id]=(scoreCount[id]||0)+1;});
        (snakeRows||[]).forEach(s=>{const id=normaliseId(s&&s.player_id);if(id&&s.confirmed)snakeCount[id]=(snakeCount[id]||0)+1;});
        (paymentRows||[]).forEach(p=>{const id=normaliseId(p&&p.player_id);if(id)paidById[id]=Math.round((parseFloat(p.paid)||0)*100);});
        (logRows||[]).forEach(row=>{
          if(!row)return;
          const note=String(row.note||'');
          if(thisRoundNotes.has(note)||!isLeagueSweepstakeMoneyNote(note))return;
          const id=normaliseId(row.player_id);
          const amt=Math.round((parseFloat(row.amount)||0)*100);
          if(id&&amt)sweepNetPence[id]=(sweepNetPence[id]||0)+amt;
        });
        const baseByLeagueId={};
        (leaguePlayers||[]).forEach(p=>{
          const id=normaliseId(p&&p.id);
          if(!id)return;
          const owedPence=1000+(Math.max(0,(scoreCount[id]||0)-8)*200)+((snakeCount[id]||0)*1000);
          baseByLeagueId[id]=(paidById[id]||0)+(sweepNetPence[id]||0)-owedPence;
        });
        const bySweepId={};
        (sw.rows||[]).forEach(r=>{
          const leaguePlayer=(!isSweepstakeLeagueGuest(r.player))?findLeagueSubmitPlayer(r.player,leaguePlayers||[],links):null;
          if(!leaguePlayer)return;
          const leagueId=normaliseId(leaguePlayer.id);
          bySweepId[normaliseId(r.id)]={leaguePlayer,basePence:baseByLeagueId[leagueId]||0,newPence:(baseByLeagueId[leagueId]||0)+(parseInt(r.net)||0)};
        });
        if(!cancelled)setSweepstakeLeagueBalancePreview({status:'ready',bySweepId});
      }catch(e){
        if(!cancelled)setSweepstakeLeagueBalancePreview({status:'error',error:e.message||String(e),bySweepId:{}});
      }
    }
    loadSweepstakeLeagueBalancePreview();
    return()=>{cancelled=true;};
  },[round&&round.id,round&&round.status,activeGroupId,sweepstakeConfig&&sweepstakeConfig.enabled,sweepstakeConfig&&sweepstakeConfig.amountPence,sweepstakeConfig&&sweepstakeConfig.scope,sweepstakeLeagueSettlement&&sweepstakeLeagueSettlement.status]);
  function SweepstakePanel({compact=false,throughHole=null,reviewTitle='',payUp=false,forceEnabled=false}){
    ...
    return <div style={{...S.card,margin:compact?'0 0 10px':16,background:payUp?'linear-gradient(135deg,rgba(245,158,11,0.30),rgba(10,21,40,0.96))':'linear-gradient(135deg,rgba(245,158,11,0.18),rgba(255,255,255,0.05))',borderColor:'rgba(245,158,11,0.55)',boxShadow:payUp?'0 14px 36px rgba(245,158,11,0.18)':'none'}}>
      ...
      {sw.final&&<>
        ...
        {sw.rows.map(r=>{
          const balancePreview=sweepstakeLeagueBalancePreview&&sweepstakeLeagueBalancePreview.bySweepId&&sweepstakeLeagueBalancePreview.bySweepId[normaliseId(r.id)];
          return <div key={r.id} style={{display:'flex',justifyContent:'space-between',gap:10,padding:'7px 0',borderTop:'1px solid rgba(255,255,255,0.08)'}}>
            <span style={{fontSize:13,color:'#fff',minWidth:0}}>
              <span style={{display:'block',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{r.name}</span>
              {balancePreview?<span style={{display:'block',fontSize:10,color:'#90ccf0',fontWeight:850,marginTop:2}}>New balance {signedMoneyFromPence(balancePreview.newPence)}</span>:payUp&&sweepstakeLeagueBalancePreview.status==='loading'?<span style={{display:'block',fontSize:10,color:'#90ccf0',fontWeight:850,marginTop:2}}>League balance checking...</span>:payUp&&sweepstakeLeagueBalancePreview.status==='ready'?<span style={{display:'block',fontSize:10,color:'rgba(255,255,255,0.55)',fontWeight:800,marginTop:2}}>Guest/unlinked - Money page unchanged</span>:null}
            </span>
            <span style={{fontSize:15,fontWeight:950,color:r.net>0?'#86efac':r.net<0?'#fca5a5':'#e5e7eb',whiteSpace:'nowrap'}}>{r.net>0?'+':''}{moneyFromPence(r.net)}</span>
          </div>;
        })}
    ...
    return <div style={{paddingBottom:40}}>
      ...
      {['FRONT 9','BACK 9'].map((label,sec)=>{const list=sec===0?front9:back9;return <div key={label}>
        ...
        {list.map((hd,i)=>{const vals=teamIds.map(id=>(holeScores[hd.hole]||{})[id]);return <div key={hd.hole} style={{display:'grid',gridTemplateColumns:'80px 1fr 1fr',minHeight:74,borderBottom:'1px solid rgba(255,255,255,0.06)',background:i%2===0?'rgba(255,255,255,0.03)':'rgba(255,255,255,0.06)'}}>
          <div style={{padding:'8px 12px',background:'rgba(0,0,0,0.18)'}}><div style={{fontSize:26,color:'#fff',fontWeight:300}}>{hd.hole}</div><div style={{fontSize:12,color:'#60b8f0'}}>Par {hd.par}</div><div style={{fontSize:11,color:'#d4af37',fontWeight:800}}>SI {hd.stroke_index}</div><div style={{fontSize:10,color:'#8ea0ad',fontWeight:850}}>{parseInt(hd.yards)||0?`${parseInt(hd.yards)} yds`:'Yds -'}</div></div>
            ...
            return(
              <div id={'hole-'+hd.hole} key={hd.hole} style={{scrollMarginTop:72,display:'grid',gridTemplateColumns:'80px '+grpPlayers.map(()=>'1fr').join(' '),borderBottom:'1px solid rgba(255,255,255,0.06)',background:idx%2===0?'rgba(255,255,255,0.03)':'rgba(0,0,0,0.1)'}}>
                <div style={{padding:'8px 12px'}}>
                  ...
                  <div style={{fontSize:9,color:'#8ea0ad',fontWeight:850}}>{parseInt(hd.yards)||0?`${parseInt(hd.yards)} yds`:'Yds -'}</div>
                </div>
                {grpPlayers.map(p=>{
...
function DayBoardsTab({rounds,scores,sb,flash,load}){
  ...
  async function settleDaySweepstakeLeagueBalances(board,linked){
    ...
    const markerNote=`Day sweepstake League balance settlement ${markerKey} | adjustment-only | v4.66`;
    ...
    const legacyMarkerNoteV457=`Day sweepstake League balance settlement ${markerKey} | adjustment-only | v4.57`;
    const legacyMarkerNoteV456=`Day sweepstake League balance settlement ${markerKey} | adjustment-only | v4.56`;
    ...
    const legacyMarkerNoteV448=`Day sweepstake League balance settlement ${markerKey} | adjustment-only | v4.48`;
    const legacyMarkerNoteV447=`Day sweepstake League balance settlement ${markerKey} | adjustment-only | v4.47`;
  ...
  async function reverseDaySweepstakeLeagueBalances(board){
    ...
    const legacyMarkerNoteV400=`Day sweepstake League balance settlement ${markerKey} | adjustment-only | v4.00`;
    const legacyMarkerNote=`Day sweepstake League balance settlement ${markerKey}`;
    const reverseNote=`Day sweepstake League balance reversal ${markerKey} | adjustment-only | v4.66`;
    ...
    const legacyReverseNoteV457=`Day sweepstake League balance reversal ${markerKey} | adjustment-only | v4.57`;
    const legacyReverseNoteV456=`Day sweepstake League balance reversal ${markerKey} | adjustment-only | v4.56`;
    ...
    const legacyReverseNoteV448=`Day sweepstake League balance reversal ${markerKey} | adjustment-only | v4.48`;
    const legacyReverseNoteV447=`Day sweepstake League balance reversal ${markerKey} | adjustment-only | v4.47`;
    ...
    const {data:logs,error:logError}=await sb.from('payment_log').select('*').or(`note.eq.${markerNote},note.eq.${legacyMarkerNoteV460},note.eq.${legacyMarkerNoteV459},note.eq.${legacyMarkerNoteV458},note.eq.${legacyMarkerNoteV457},note.eq.${legacyMarkerNoteV456},note.eq.${legacyMarkerNoteV455},note.eq.${legacyMarkerNoteV454},note.eq.${legacyMarkerNoteV453},note.eq.${legacyMarkerNoteV452},note.eq.${legacyMarkerNoteV451},note.eq.${legacyMarkerNoteV450},note.eq.${legacyMarkerNoteV449},note.eq.${legacyMarkerNoteV448},note.eq.${legacyMarkerNoteV447},note.eq.${legacyMarkerNoteV446},note.eq.${legacyMarkerNoteV445},note.eq.${legacyMarkerNoteV444},note.eq.${legacyMarkerNoteV443},note.eq.${legacyMarkerNoteV442},note.eq.${legacyMarkerNoteV441},note.eq.${legacyMarkerNoteV440},note.eq.${legacyMarkerNoteV439},note.eq.${legacyMarkerNoteV438},note.eq.${legacyMarkerNoteV437},note.eq.${legacyMarkerNoteV436},note.eq.${legacyMarkerNoteV435},note.eq.${legacyMarkerNoteV434},note.eq.${legacyMarkerNoteV433},note.eq.${legacyMarkerNoteV432},note.eq.${legacyMarkerNoteV431},note.eq.${legacyMarkerNoteV430},note.eq.${legacyMarkerNoteV429},note.eq.${legacyMarkerNoteV428},note.eq.${legacyMarkerNoteV420},note.eq.${legacyMarkerNoteV419},note.eq.${legacyMarkerNoteV400},note.eq.${legacyMarkerNote}`);
    if(logError)throw logError;
