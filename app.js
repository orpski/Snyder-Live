...
async function sendSnyderLiveNotification(type,payload){
  try{
    ...
    const body={type,app:'snyder-live',subscriptionTable:SNYDER_PUSH_TABLE,version:'v2.41',createdAt:new Date().toISOString(),...(payload||{})};
      ...
      <div style={{minHeight:'100vh',paddingBottom:40}}>
        ...
        <div style={{padding:16}}>
            ...
            <div style={{padding:'12px 14px',background:setup.matchplay&&setup.matchplay.enabled?'rgba(96,184,240,0.13)':'rgba(255,255,255,0.06)',borderRadius:10,marginBottom:16,border:'1px solid '+(setup.matchplay&&setup.matchplay.enabled?'rgba(96,184,240,0.38)':'rgba(255,255,255,0.12)')}}>
              ...
              {setup.matchplay&&setup.matchplay.enabled&&<div style={{marginTop:10,display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                <div style={{gridColumn:'1 / -1',display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:2}}>
                  <button onClick={()=>setMatchplayMode('doubles')} style={{border:'1px solid '+(((setup.matchplay&&setup.matchplay.mode)||'doubles')==='doubles'?'rgba(96,184,240,0.6)':'rgba(255,255,255,0.12)'),background:(((setup.matchplay&&setup.matchplay.mode)||'doubles')==='doubles')?'rgba(96,184,240,0.20)':'rgba(255,255,255,0.06)',color:'#fff',borderRadius:10,padding:'9px 8px',fontSize:12,fontWeight:900}}>Doubles Stableford</button>
                  <button onClick={()=>setMatchplayMode('foursomes')} style={{border:'1px solid '+((setup.matchplay&&setup.matchplay.mode)==='foursomes'?'rgba(251,191,36,0.6)':'rgba(255,255,255,0.12)'),background:(setup.matchplay&&setup.matchplay.mode)==='foursomes'?'rgba(251,191,36,0.20)':'rgba(255,255,255,0.06)',color:'#fff',borderRadius:10,padding:'9px 8px',fontSize:12,fontWeight:900}}>Foursomes</button>
                </div>
                {(setup.matchplay&&setup.matchplay.mode)==='foursomes'&&<>
                  ...
                  <select value={(parseInt(setup.matchplay&&setup.matchplay.teamAShots)||0)>0?'A':((parseInt(setup.matchplay&&setup.matchplay.teamBShots)||0)>0?'B':'none')} onChange={e=>{const team=e.target.value;setSetup(q=>{const clean=cleanMatchplaySetup(q.matchplay||{},participants);const shotCount=Math.max(parseInt(clean.teamAShots)||0,parseInt(clean.teamBShots)||0,0);return {...q,matchplay:{...clean,enabled:true,mode:'foursomes',teamAShots:team==='A'?shotCount:0,teamBShots:team==='B'?shotCount:0}};});}} style={{...S.inp,marginBottom:0,padding:'9px 10px',fontSize:13}}>
                  ...
                  <input type='number' min='0' value={Math.max(parseInt(setup.matchplay&&setup.matchplay.teamAShots)||0,parseInt(setup.matchplay&&setup.matchplay.teamBShots)||0)} onChange={e=>{const n=Math.max(0,parseInt(e.target.value)||0);setSetup(q=>{const clean=cleanMatchplaySetup(q.matchplay||{},participants);const receiver=(parseInt(clean.teamAShots)||0)>0?'A':((parseInt(clean.teamBShots)||0)>0?'B':'none');return {...q,matchplay:{...clean,enabled:true,mode:'foursomes',teamAShots:receiver==='A'?n:0,teamBShots:receiver==='B'?n:0}};});}} placeholder='Shots in the match' style={{...S.inp,marginBottom:0,padding:'9px 10px',fontSize:13}}/>
                  <div style={{gridColumn:'1 / -1',fontSize:11,color:'rgba(255,255,255,0.72)',lineHeight:1.35}}>These two team names are the players on the scorecard. Only one side can receive shots. Pick “No shots given” and leave shots as 0 for a scratch match.</div>
                </>}
          ...
          {isFoursomesSetup()&&<div style={{...S.card,marginBottom:12,background:'rgba(251,191,36,0.09)',borderColor:'rgba(251,191,36,0.28)'}}>
            ...
            <div style={{fontSize:11,color:'#fbbf24',lineHeight:1.35}}>Do not add four players here. The two team names above become the two score columns. You will score one ball for each team.</div>
            <div style={{fontSize:11,color:'#90ccf0',lineHeight:1.35,marginTop:6}}>Scorer: <b>{currentUser&&currentUser.display_name||'you'}</b></div>
          </div>}
...
function LiveScorecard({round,group,players,courses,rounds,scores,sb,flash,load,setView,holeScores,setHoleScores,currentUser}){
  ...
  function FoursomesScorecard(){
    ...
    return <div style={{paddingBottom:40}}>
      ...
      {['FRONT 9','BACK 9'].map((label,sec)=>{const list=sec===0?front9:back9;return <div key={label}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'80px 1fr 1fr',padding:'8px 12px',borderBottom:'1px solid rgba(255,255,255,0.1)',background:'rgba(0,50,120,0.4)',gap:6,alignItems:'center'}}>
  ...
  return(
    <div style={{minHeight:'100vh',background:'linear-gradient(160deg,#0a1528 0%,#0d2040 50%,#0a1830 100%)',overflowX:'hidden',touchAction:inputHole?'none':'auto'}}>
      ...
      {!(matchplayConfig&&matchplayConfig.enabled&&matchplayConfig.mode==='foursomes')&&['FRONT 9','BACK 9'].map((label,sec)=>(
        <div key={label}>
            ...
            {overallMode==='cupOverall'&&(()=>{const s=cupIfItStaysScore();const rows=cupOverallSinglesRows();return <>
              ...
              {rows.map((r,idx)=>(
                <div key={r.id} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',background:idx===0?'linear-gradient(135deg,rgba(251,191,36,0.26),rgba(255,255,255,0.07))':idx===1?'linear-gradient(135deg,rgba(203,213,225,0.20),rgba(255,255,255,0.06))':idx===2?'linear-gradient(135deg,rgba(180,83,9,0.22),rgba(255,255,255,0.06))':'rgba(255,255,255,0.06)',border:'1px solid '+(idx<3?'rgba(251,191,36,0.24)':'rgba(255,255,255,0.08)'),borderRadius:12,marginBottom:8}}>
                  <div style={{width:34,textAlign:'center',fontSize:idx<3?13:18,color:idx===0?'#fbbf24':idx===1?'#cbd5e1':idx===2?'#fb923c':'rgba(255,255,255,0.48)',fontWeight:950,lineHeight:1.05}}>{idx<3?EMOJI.trophy+' '+(idx+1):idx+1}</div>
                  <div style={{flex:1,minWidth:0}}>
