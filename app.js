...
const GOLF_API='UHWIG5FFJSUFGN2XEMUQWFVLBE';
// =========================================================
...
function App(){
  ...
  const[appData,setAppData]=useState({players:[],courses:[],rounds:[],groups:[],competitions:[],scores:[],cupUsers:[],guests:[]});
  ...
  async function loadAll(){
    const[players,courses,rounds,groups,competitions,scores,cupUsers,guests]=await Promise.all([
    ...
    setAppData({players,courses,rounds,groups,competitions,scores,cupUsers,guests});
  }
    ...
    return(
      <div style={{...S.card,marginBottom:8,cursor:'pointer',opacity:0.9}} onClick={()=>openRound(rd)}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <div>
            <div style={{fontSize:14,color:'#fff',fontWeight:500}}>{rd.name||rd.course_name}</div>
            <div style={{fontSize:11,color:'#60b8f0'}}>Round started: {formatRoundStart(rd)}</div>
          </div>
          <div style={{fontSize:11,color:'#1b5e20',background:'rgba(27,94,32,0.15)',borderRadius:6,padding:'3px 8px',fontWeight:600}}>Completed</div>
        </div>
      </div>
    );
  }
                ...
                return(
                  <div key={rd.id} onClick={()=>{window.history.replaceState({view:'home'},'',null);setSelectedRound(rd);setView('live');}} style={{minWidth:200,borderRadius:14,overflow:'hidden',background:'rgba(255,255,255,0.07)',border:'1px solid rgba(255,255,255,0.1)',flexShrink:0,cursor:'pointer'}}>
                    <div style={{height:110,background:'linear-gradient(135deg,#0a3d6b 0%,#1a5fa8 60%,#0070BB 100%)',display:'flex',alignItems:'center',justifyContent:'center',position:'relative'}}>
                      <div style={{fontSize:40,opacity:0.3}}>G</div>
            ...
            return(
              <div key={rd.id} style={{...S.card,marginBottom:16,cursor:'pointer',borderColor:'rgba(239,68,68,0.3)'}} onClick={()=>openRound(rd)}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:10,marginBottom:10}}>
                  <div>
                    <div style={{fontSize:16,color:'#fff',fontWeight:700}}>{rd.name||rd.course_name}</div>
                    <div style={{fontSize:11,color:'#60b8f0'}}>Round started: {formatRoundStart(rd)}</div>
                  </div>
                  <div style={{fontSize:10,color:'#fff',background:'#ef4444',borderRadius:20,padding:'4px 8px',fontWeight:700,letterSpacing:'0.08em'}}>LIVE</div>
                </div>
...
function LiveScorecard({round,group,players,courses,sb,flash,load,setView,holeScores,setHoleScores,currentUser}){
  ...
  const playingHcps=group&&group.playing_handicaps||{};
  const participants=group&&group.participants||[];
  const grpPlayers=participants.length>0?participants:(group&&group.player_ids||[]).map(id=>{
    const p=players.find(pl=>pl.id===id);
    return p||{id,name:'Player',display_name:'Player',current_handicap:playingHcps[id]||0};
  });

  // Determine if current user can edit
  const playerIds=(group&&group.player_ids)||grpPlayers.map(p=>p.id);
  const canEdit=!round._spectator&&isLiveRound(round)&&currentUser&&(playerIds.includes(currentUser.id)||grpPlayers.some(p=>p.id===currentUser.id));
  ...
  // ---------------------------------------------------------
  async function autoSave(holeNum,pid,val){
    ...
    const pts=val===-1?0:calcStableford(val,hd.par,hd.stroke_index,hcp)||0;
    ...
    // Save to Supabase
    const{error}=await sb.from('cup_scores').upsert({
      round_id:round.id,player_id:pid,hole_number:holeNum,
      gross_score:val,stableford_points:pts,par:hd.par,stroke_index:hd.stroke_index,
  ...
  function setScore(holeNum,pid,val){
    setHoleScores(prev=>{
      ...
      const allScored=grpPlayers.every(p=>(updated[holeNum]||{})[p.id]!==undefined);
      if(allScored){
        setTimeout(()=>flash('Hole '+holeNum+' saved'),200);
    ...
    autoSave(holeNum,pid,val).catch(e=>flash('Save error: '+e.message,'error'));
  }
    ...
    try{
      ...
      for(const[hNum,pMap]of Object.entries(holeScores)){
        for(const[pid,gross]of Object.entries(pMap)){
          ...
          const hd=getHole(parseInt(hNum));
          const hcp=parseFloat(playingHcps[pid]||0);
          const pts=gross===-1?0:calcStableford(gross,hd.par,hd.stroke_index,hcp)||0;
          await sb.from('cup_scores').upsert({
            round_id:round.id,player_id:pid,hole_number:parseInt(hNum),
    ...
    <div style={{minHeight:'100vh',background:'linear-gradient(160deg,#0a1528 0%,#0d2040 50%,#0a1830 100%)'}}>
      <div style={{position:'sticky',top:0,zIndex:10,background:'linear-gradient(160deg,#0a1528,#0d2040)',borderBottom:'2px solid #0070BB'}}>
        ...
        <div style={{padding:'10px 14px',display:'flex',alignItems:'center',gap:8}}>
          ...
          <div style={{width:38,height:38,borderRadius:8,overflow:'hidden',flexShrink:0,background:'linear-gradient(135deg,#0a3d6b,#0070BB)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:9,color:'rgba(255,255,255,0.7)',border:'1px solid rgba(255,255,255,0.15)'}}>
            {course&&course.image_url
              ?<img src={course.image_url} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}} onError={e=>{e.target.style.display='none';}}/>
              :<div style={{textAlign:'center',lineHeight:1.2,padding:2}}>{(course&&course.name||'?').split(' ').map(w=>w[0]).join('').slice(0,4)}</div>
            }
          </div>
          <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'80px '+grpPlayers.map(()=>'1fr').join(' '),padding:'4px 12px',borderTop:'1px solid rgba(255,255,255,0.1)',background:'rgba(0,50,120,0.4)',gap:6,alignItems:'center'}}>
          ...
          {grpPlayers.map(p=>(
            <div key={p.id} style={{textAlign:'center',padding:'3px 0'}}>
              <div style={{fontSize:13,color:'#fff',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{((p.name||p.display_name)||'?').split(' ')[0]}</div>
              <div style={{fontSize:9,color:'#60b8f0'}}>HCP {playingHcps[p.id]||0}</div>
            </div>
          ))}
        </div>
      </div>
                ...
                {grpPlayers.map(p=>{
                  ...
                  return(
                    <div key={p.id} onClick={()=>{
                      ...
                      {gross>0||gross===-1?(
                        <div>
                          <div style={{fontSize:28,color:'#fff',lineHeight:1,textAlign:'center'}}>{gross===-1?'0':gross}</div>
                          {pts!==null&&<div style={{position:'absolute',top:6,right:8,fontSize:11,color:'rgba(255,255,255,0.9)',background:'rgba(0,0,0,0.3)',borderRadius:4,padding:'1px 5px'}}>{pts}pt</div>}
                          {gross>0&&<div style={{position:'absolute',bottom:5,right:7,fontSize:11,color:'rgba(255,255,255,0.7)'}}>{running}</div>}
                        </div>
                      ):(
...
// =========================================================
function AdminPanel({courses,rounds,groups,sb,flash,setView,load,cupUsers}){
  ...
  return(
    <div>
      <div style={{display:'flex',gap:8,marginBottom:16}}>
        <button onClick={()=>setShowSearch(s=>!s)} style={{...S.pri,flex:1,fontSize:13}}>Search API</button>
...
// =========================================================
function TournamentsView({competitions,rounds,groups,scores,players,courses,sb,flash,setView,load,activeComp,selectedComp,setSelectedComp,currentUser,isAdmin}){
